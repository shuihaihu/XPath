...
int local_OptIP(int s, int *IP, int (*ST)[TotalTNum]){
  int blocksz[Pmax+1];
  int porder[Pmax+1];
  int *flag=new int[TotalTNum];
  memset(flag,0, sizeof(int)*TotalTNum);
...
int compute_ORT(int s, int *IP, ORTC_node *ORTC_tree, 
					  int ORTC_nnum, int ORTC_leafnnum, int (*ST)[TotalTNum]){
 ...
 for(int node=1; node<=ORTC_nnum; node++){
   ...
   if(hasdupp==1){
     ...
     for(int bit=0; bit<=Pmax; bit++)
       ORTC_tree[node].routemap[bit]=OORTC_tree[node].routemap[bit] | RTC_tree[parent].routemap[bit];
...
int get_maxORT(int KeySwitch, int *IP, int ORTC_nnum, int ORTC_leafnnum, 
    ORTC_node *ORTC_tree, int (*ST)[TotalTNum]){
			int answer=0;

			local_OptIP(KeySwitch, IP, ST);
...
int CCencoding_ORTC(int (*ST)[TotalTNum], ofstream &output){
  ...
  ORTC_tree=new ORTC_node[ORTC_nnum];
	...
	KeySwitch=0;
	temp=get_maxORT(KeySwitch, IP, ORTC_nnum, ORTC_leafnnum, ORTC_tree, ST);
	if(temp<FinalAnswer){
		FinalAnswer=temp;
