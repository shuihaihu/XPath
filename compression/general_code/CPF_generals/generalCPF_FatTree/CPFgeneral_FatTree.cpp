...
int main(){
	...
	/*ofstream output("FatTree_Output.txt");

	for(int i=0; i<TotalPathNum; i++){
		PathNode *temp=PathInfo[i].head;
		output<<"Path "<<i<<endl;
		while(temp!=NULL){
			output<<temp->node<<" "<<temp->port<<"-->";
			temp=temp->next;
		}
		output<<endl<<endl;
	}*/ 
	...
	/*for(int i=0; i<srcNum; i++)
		for(int j=0; j<destNum; j++){
			SrcDestGroup[i][j].head=NULL;
			SrcDestGroup[i][j].tail=NULL;
		}*/
